...
const ProfileScreen = () => {
  ...
  const account: MockAccount = useMemo(() => {
    return mockAccounts[activeUsername] || mockAccounts["b4by_4ngel__"];
  ...
  const userPosts = useMemo(() => {
    ...
    return account.posts.map((post, i) => ({
      ...
      isReel: true,
      views: [37700, 170, 2146, 1200, 458, 89, 234, 567, 123][i] || Math.floor(Math.random() * 500 + 50),
          ...
          {post.isReel && (
            <div className="absolute top-[6px] right-[6px]">
              <svg width="22" height="22" viewBox="0 0 24 24" className="drop-shadow-lg">
                <defs>
                  <mask id="reels-icon-mask">
                    <rect width="24" height="24" fill="white"/>
                    <path d="M9.5 7.5v9l7-4.5-7-4.5z" fill="black"/>
                  </mask>
                </defs>
                <rect x="2" y="2" width="20" height="20" rx="4" fill="white" mask="url(#reels-icon-mask)"/>
              </svg>
            </div>
          )}
          <div className="absolute bottom-[5px] left-[6px] flex items-center gap-[3px]">
            <svg width="15" height="13" viewBox="0 0 120 100" className="drop-shadow-lg">
              <defs>
                <mask id={`eye-mask-post-${post.views}`}>
              ...
              <circle cx="60" cy="62" r="30" fill="white" mask={`url(#eye-mask-post-${post.views})`}/>
            </svg>
            <span className="text-[11px] font-semibold text-white drop-shadow-lg">{formatCount(post.views)}</span>
          </div>
        </>
      )}
      {clean && (
        <div className="absolute bottom-[5px] left-[6px] flex items-center gap-[3px]">
           <svg width="15" height="13" viewBox="0 0 120 100" className="drop-shadow-lg">
              ...
              {/* Eyelid arc */}
              <path d="M15 45 C30 8, 90 8, 105 45" stroke="white" strokeWidth="10" strokeLinecap="round" fill="none"/>
              {/* Eyeball with crescent cutout */}
              <circle cx="60" cy="62" r="30" fill="white" mask={`url(#eye-mask-${post.views})`}/>
            </svg>
          <span className="text-[11px] font-semibold text-white drop-shadow-lg">{formatCount(post.views)}</span>
        </div>
      )}
  ...
  return (
    ...
    >
      ...
      <div className="fixed top-0 left-0 right-0 z-50 mx-auto max-w-lg bg-background">
        <header className="grid grid-cols-[40px_1fr_auto] items-center bg-background px-4 py-4 border-b border-border/30">
        <button className="text-foreground justify-self-start relative">
          <Plus size={28} strokeWidth={1.5} />
          <div className="absolute top-1 right-0 h-[8px] w-[8px] rounded-full bg-[hsl(var(--ig-like))]" />
        </button>
      ...
      <div className="px-4 pt-0 overflow-visible">
        <div className="flex items-center gap-5 overflow-visible">
          <div className="relative flex-shrink-0 flex flex-col items-center mt-10">
            ...
            {account.storyNote && (
              <div className="absolute -top-[28px] left-1/2 -translate-x-1/2 z-10 w-max">
                <div className="bg-background dark:bg-[#262626] rounded-[18px] px-3 py-1.5 max-w-[130px] shadow-sm border border-border/60 dark:border-[#363636]">
                  <p className="text-[13px] text-foreground text-center leading-snug break-words">{account.storyNote}</p>
                </div>
                {/* Triangle pointer */}
            ...
            <div className="rounded-full border-[1.5px] border-muted-foreground/30 p-[3px]">
              <img src={profile.avatar} alt={profile.username} className="h-[80px] w-[80px] rounded-full object-cover" />
            </div>
            <div className="absolute bottom-[6px] right-[1px] flex h-[22px] w-[22px] items-center justify-center rounded-full border-[2px] border-background bg-[#0095f6] text-white">
        ...
        <div className="mt-1.5">
          {account.category && <p className="text-[13px] text-muted-foreground">{account.category}</p>}
          <p className="mt-0.5 text-[13px] text-foreground leading-[18px] whitespace-pre-line">{profile.bio.replace(`${account.category}\n`, '')}</p>
          {profile.website && (
            <div className="mt-0.5 flex items-center gap-1">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="hsl(225 73% 57%)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" />
                <path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" />
              </svg>
              <span className="text-[13px] text-[hsl(225,73%,57%)] font-medium">{profile.website}</span>
            </div>
          )}
        </div>
        ...
        <div
          className="mt-3 rounded-xl bg-secondary/80 p-3.5 w-full text-left border border-border/40"
        ...
        >
          <div className="flex items-center justify-between">
            <p className="text-[14px] font-semibold text-foreground">Professional dashboard</p>
          </div>
          <div className="flex items-center gap-1.5 mt-0.5">
            ...
            <p className="text-[12px] text-muted-foreground">{account.dashboardViews || "37.3K"} views in the last 30 days.</p>
          </div>
        </div>
        ...
        <div className="mt-3 flex gap-1.5">
          <Button variant="secondary" className="flex-1 h-[34px] text-[13px] font-semibold rounded-md" onClick={() => setEditOpen(true)}>
            Edit profile
          </Button>
          <Button variant="secondary" className="flex-1 h-[34px] text-[13px] font-semibold rounded-md" onClick={() => { navigator.clipboard?.writeText(window.location.origin + "/profile"); toast.success("Profile link copied!"); }}>
            Share profile
          </Button>
        </div>
      </div>
      ...
      <div className="flex gap-4 overflow-x-auto px-4 py-4 hide-scrollbar">
        ...
        />
        <button onClick={handleAddHighlight} className="flex flex-col items-center gap-1 min-w-[76px]">
          <div className="flex h-[76px] w-[76px] items-center justify-center rounded-full border-[2px] border-foreground/60">
            <Plus size={32} className="text-foreground" strokeWidth={1.2} />
          </div>
        ...
        {highlights.map((h, idx) => (
          <div
            key={`${h.name}-${idx}`}
            className={cn("flex flex-col items-center gap-1 min-w-[76px] select-none cursor-pointer", pressingHighlight === idx && "scale-95 transition-transform")}
            style={{ WebkitTouchCallout: "none" } as React.CSSProperties}
            onContextMenu={(e) => e.preventDefault()}
          ...
          >
            <div className="h-[76px] w-[76px] rounded-full overflow-hidden border-[1.5px] border-foreground/30 p-[2.5px]">
              <img src={h.image} alt={h.name} className="h-full w-full rounded-full object-cover pointer-events-none" draggable={false} />
            </div>
            <span className="text-[11px] text-foreground max-w-[76px] truncate">{h.name}</span>
          </div>
        ))}
      </div>
      ...
      {activeTab === "posts" && (
        <div className="grid grid-cols-3 gap-[1.5px]">
          {userPosts.map((post, i) => renderGridItem(post, i, "aspect-[3/4]"))}
          {/* Add new reel button for just4abhii */}
          {isJust4abhii && (
            <button
              onClick={handleAddReel}
              className="relative aspect-[3/4] overflow-hidden bg-secondary flex flex-col items-center justify-center gap-2"
            >
      ...
      {activeTab === "reels" && (
         <div className="grid grid-cols-3 gap-[1.5px] dark:gap-[3px]">
          {userPosts.filter(p => p.isReel).map((post, i) => renderGridItem(post, i, "aspect-[9/16]", true))}
          {isJust4abhii && (
            <button
              onClick={handleAddReel}
              className="relative aspect-[9/16] overflow-hidden bg-secondary flex flex-col items-center justify-center gap-2"
            >
              <Plus size={32} className="text-muted-foreground" />
              <span className="text-[11px] text-muted-foreground font-medium">Add Reel</span>
            </button>
